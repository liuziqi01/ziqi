...
console.log('AI search para passed depth' + depth +' alpha ' + alpha + ' beta '+ beta+ ' position ' + positions + ' cutoffs ' + cutoffs  );
  ...
  if (this.grid.playerTurn) {
    bestScore = alpha;
    for (var direction in [0, 1, 2, 3, 4, 5]) {
	
	console.log('calling clone now');
      var newGrid = this.grid.clone();
	 console.log('typeof newGrid ?' + typeof newGrid)
      if (newGrid.move(direction).moved) {
        positions++;
		console.log("positions-" + positions);
      
		if (newGrid.isWin()) {
		console.log('isWin() returned ');
