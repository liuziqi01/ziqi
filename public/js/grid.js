...
for ( var x = 0; x< 3; x ++)
{
	...
	for( var z = 0; z < 3; z ++)
            {
				console.log(' indexes ' + x , y , z);
				Grid.prototype.indexes[x][y].push({x:x, y:y, z:z});
				console.log("check 2 " + x, y);
			}
    }
}
...
Grid.prototype.randomAvailableCell = function () {
  var cells = this.availableCells();
    console.log("randomAvailiabelCell");
  if (cells.length) {
...
Grid.prototype.clone = function()
{
  ...
  for (var x = 0; x < this.size; x++) {
	...
	for(var z = 0; z < this.size ; z++){
      if (this.cells[x][y][z]) {
        newGrid.insertTile(this.cells[x][y][z].clone());
      }
      }
    }
  }
...
Grid.prototype.moveTile = function (tile, cell) {
  this.cells[tile.x][tile.y][tile.z] = null;
  this.cells[cell.x][cell.y][cell.z] = tile;
...
Grid.prototype.tileMatchesAvailable = function () {
  ...
  for (var x = 0; x < this.size; x++) {
    for (var y = 0; y < this.size; y++) {
	  for (var z = 0; z < this.size; z++) {
      	tile = this.cellContent({ x: x, y: y, z: z });
...
Grid.prototype.islands = function() {
  var self = this;
    var mark = function(x, y ,z , value) {
    if (x >= 0 && x <= 3 && y >= 0 && y <= 3 && z >= 0 && z<=3 &&
        self.cells[x][y][z] &&
        self.cells[x][y][z].value == value &&
        !self.cells[x][y][z].marked ) {
      self.cells[x][y][z].marked = true;
      
      for (direction in [0,1,2,3]) {
        var vector = self.getVector(direction);
          mark(x + vector.x, y + vector.y,z+vector.z, value);
      }
    }
  }
  ...
  for (var x=0; x<3; x++) {
	...
	{
      if (this.cells[x][y][z] &&
          !this.cells[x][y][z].marked) {
        islands++;
          mark({ x:x, y:y,z:z }, this.cells[x][y][z].value);
      }
	}
    }
  }
...
Grid.prototype.smoothness = function() {
  ...
  for (var x=0; x<3; x++)
 {
	...
	for(var z =0; z<3; z++)
	 {
      if ( this.cellOccupied( this.indexes[x][y][z])) {
        var value = Math.log(this.cellContent( this.indexes[x][y][z] ).value) / Math.log(2);
        for (var direction=1; direction<=2; direction++) {
          var vector = this.getVector(direction);
          var targetCell = this.findFarthestPosition(this.indexes[x][y][z], vector).next;
...
Grid.prototype.monotonicity2 = function() {
  ...
  for (var x=0; x<3; x++) {
    ...
    var next = current+1;
    while ( next<3 ) {
      while ( next<3 && !this.cellOccupied( this.indexes[x][next][z] )) {
        next++;
      }
      if (next>=3) { next--; }
	var currentValue = this.cellOccupied({x:x, y:current,z:z}) ?
        Math.log(this.cellContent( this.indexes[x][current][z] ).value) / Math.log(2) :
        0;
	var nextValue = this.cellOccupied({x:x, y:next,z:z}) ?
        Math.log(this.cellContent( this.indexes[x][next][z] ).value) / Math.log(2) :
        0;
  ...
  for (var y=0; y<3; y++) {
    ...
    var next = current+1;
    while ( next<3 ) {
      while ( next<3 && !this.cellOccupied( this.indexes[next][y][z] )) {
        next++;
      }
      if (next>=3) { next--; }
	var currentValue = this.cellOccupied({x:current, y:y,z:z}) ?
        Math.log(this.cellContent( this.indexes[current][y][z] ).value) / Math.log(2) :
        0;
	var nextValue = this.cellOccupied({x:next, y:y,z:z}) ?
        Math.log(this.cellContent( this.indexes[next][y][z] ).value) / Math.log(2) :
        0;
      if (currentValue > nextValue) {
  ...
  for (var y=0; y<3; y++) {
    ...
    var next = current+1;
    while ( next<3 ) {
      while ( next<3 && !this.cellOccupied( this.indexes[x][y][next] )) {
        next++;
      }
      if (next>=3) { next--; }
	var currentValue = this.cellOccupied({x:x, y:y, z:current}) ?
        Math.log(this.cellContent( this.indexes[x][y][current] ).value) / Math.log(2) :
        0;
	var nextValue = this.cellOccupied({x:x, y:y,z:next}) ?
        Math.log(this.cellContent( this.indexes[x][y][next] ).value) / Math.log(2) :
        0;
      if (currentValue > nextValue) {
    ...
    return Math.max(totals[0], totals[1]) + Math.max(totals[2], totals[3]) + Math.max(totals[4],totals[5]);
}
...
Grid.prototype.maxValue = function() {
  ...
  for (var x=0; x<3; x++) {
	...
	for(var z=0;z<this.size ; z++){
      if (this.cellOccupied(this.indexes[x][y][z])) {
        var value = this.cellContent(this.indexes[x][y][z]).value;
        if (value > max) {
          max = value;
        }
      }
...
Grid.prototype.isWin = function() {
  ...
  for (var x=0; x<3; x++) {
    for (var y=0; y<3; y++) {
	for(var z=0; z<3 ; z++) {
//	console.log('xyz in isWin() ' + x,y,z);
//	console.log('typeof this ' + typeof this + ' this- ' + this)
	var testTemp = this.indexes[x][y][z];
      if (self.cellOccupied(this.indexes[x][y][z])) {
        if (self.cellContent(this.indexes[x][y][z]).value == 2048) {
          return true;
        }
      }
	}
